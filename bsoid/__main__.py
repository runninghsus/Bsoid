if __name__ == "__main__":
    ...
    st.subheader(
        'Extract behavior from pose for any organism, any camera angle! Keeping boxes unchecked when not needed decreases processing time.')
    ...
    if not last_run:
        ...
        st.subheader('LOAD DATA and SETTINGS')
        st.write('__Preferably a diverse set with total time > 3 hours.__')
        ...
        st.write('For each run, computed variables are stored as __.sav files__. for you to reload them afterwards '
                 'If you type in the same variable prefix as last run, this will overwrite the pre-existing workspace.')
        ...
        if st.checkbox('Show machine learning confidence in identified points?', False):
            st.write('If the below __% time occluded (y-axis) is unreasonable, consider re-train pose-estimation software (DeepLabCut/LEAP).__')
    ...
    st.subheader('GENERATE CLUSTERS and TRAIN MACHINE LEARNING CLASSIFIER')
    ...
    # HDBSCAN
    st.subheader('Provide identity to clusters.')
    st.text_area('', '''
    The following slider allows you to adjust cluster number based on size.
    The preset (0.4-1.2%) usually identifies > 4 and < 40 clusters. 
    ''')
    ...
    st.markdown('Your minimum cluster size ranges between **{}%** and **{}%**, '
                'which is equivalent to roughly {} seconds '
                'for the smallest possible cluster.'.format(cluster_range[0], cluster_range[1],
                                                            round(cluster_range[0] * 0.01 * umap_embeddings.shape[0])))
    ...
    if st.checkbox("Show clustering plot?", True):
        with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_feats.sav'))), 'rb') as fr:
            f_10fps, f_10fps_sc, umap_embeddings = joblib.load(fr)
        with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_clusters.sav'))), 'rb') as fr:
            assignments, soft_clusters, soft_assignments = joblib.load(fr)
    ...
    st.subheader('Based on __soft__ assignment, train a random forest classifier to _learn_ the rules.')
    st.text_area('', '''
    Random forest classifier will be trained on recognizing distance, angles, and speed. 
    This is for our vision in closed-loop experiments
                 ''')
    if st.button("Start training a behavioral random forest classifier"):
        with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_feats.sav'))), 'rb') as fr:
            f_10fps, f_10fps_sc, umap_embeddings = joblib.load(fr)
        with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_clusters.sav'))), 'rb') as fr:
            assignments, soft_clusters, soft_assignments = joblib.load(fr)
        ...
        st.info(
            'Training random forest classifier on randomly partitioned {}% of training data...'.format(
                (1 - HLDOUT) * 100))
        ...
        st.info('Done training random forest classifier '
        ...
        with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_randomforest.sav'))), 'wb') as f:
            joblib.dump([feats_test, labels_test, classifier, clf, scores, nn_assignments], f)
        st.balloons()
    ...
    if last_run:
        ...
        with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_randomforest.sav'))), 'rb') as fr:
            feats_test, labels_test, classifier, clf, scores, nn_assignments = joblib.load(fr)
        st.markdown('**_CHECK POINT_**: Done training random forest classifier'
                    'mapping **{}** features to **{}** assignments.'.format(f_10fps.T.shape, soft_assignments.T.shape))

    if st.checkbox("Show confusion matrix on {}% held-out test data?".format(HLDOUT * 100), False):
        # with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_neuralnet.sav'))), 'rb') as fr:
        #     feats_test, labels_test, classifier, clf, scores, nn_assignments = joblib.load(fr)
        with open(os.path.join(OUTPUT_PATH, str.join('', (MODEL_NAME, '_randomforest.sav'))), 'rb') as fr:
            feats_test, labels_test, classifier, clf, scores, nn_assignments = joblib.load(fr)
    ...
    if st.button('Export'):
        ...
        if any('Feature corresponding labels' in o for o in result1_options):
            ...
            micolumns = pd.MultiIndex.from_tuples([('UMAP embeddings', 'Dimension 1'), ('', 'Dimension 2'),
                                                   ('', 'Dimension 3'), ('HDBSCAN', 'Assignment No.'),
                                                   ('HDBSCAN*SOFT', 'Assignment No.'), ('Random Forest', 'Assignment No.')],
                                                  names=['Type', 'Frame@10Hz'])
    ...
    if st.sidebar.checkbox('Behavioral structure visual analysis?', False):
        ...
        st.subheader('GENERATE VIDEOS and PREDICT LABELS')
